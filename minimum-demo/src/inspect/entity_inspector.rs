        ...
        T: Component + imgui_inspect::InspectRenderDefault<T>
{
    fn render(&self, resource_map: &ResourceMap, entity_handles: &[EntityHandle], ui: &imgui::Ui) {
        ...
        if data.len() > 0 {
            <T as imgui_inspect::InspectRenderDefault<T>>::render(data.as_slice(), "label", ui, &InspectArgsDefault::default());
        }
    }
    ...
    fn render_mut(&self, resource_map: &ResourceMap, entity_handles: &[EntityHandle], ui: &imgui::Ui) {
        ...
        if data.len() > 0 {
            <T as imgui_inspect::InspectRenderDefault<T>>::render_mut(data.as_mut_slice(), "label", ui, &InspectArgsDefault::default());
        }
    }
}
...
trait RegisteredComponentPrototypeTrait: Send + Sync {
    fn render(&self, prototypes: &HashMap::<core::any::TypeId, Vec<&Box<PersistentComponentPrototype>>>, ui: &imgui::Ui);
    fn render_mut(&self, prototypes: &mut HashMap::<core::any::TypeId, Vec<&mut Box<PersistentComponentPrototype>>>, ui: &imgui::Ui);
}
...
impl<T> RegisteredComponentPrototypeTrait for RegisteredComponentPrototype<T>
    where
        T: PersistentComponentPrototype + imgui_inspect::InspectRenderDefault<T> + named_type::NamedType
{
    fn render(&self, prototypes: &HashMap::<std::any::TypeId, Vec<&Box<PersistentComponentPrototype>>>, ui: &imgui::Ui) {
        if let Some(values) = prototypes.get(&std::any::TypeId::of::<T>()) {
            ...
            <T as imgui_inspect::InspectRenderDefault<T>>::render(&cast_values, T::type_name(), ui, &InspectArgsDefault::default());
        }
    }

    fn render_mut(&self, prototypes: &mut HashMap::<std::any::TypeId, Vec<&mut Box<PersistentComponentPrototype>>>, ui: &imgui::Ui) {
        if let Some(values) = prototypes.get_mut(&std::any::TypeId::of::<T>()) {
            ...
            <T as imgui_inspect::InspectRenderDefault<T>>::render_mut(&mut cast_values, T::type_name(), ui, &InspectArgsDefault::default());
        }
    }
...
impl InspectorRegistry {
    ...
    pub fn register_component_prototype<T: crate::constructors::PersistentComponentPrototype + 'static + imgui_inspect::InspectRenderDefault<T>>(&mut self) {
    ...
    pub fn render_mut(
        ...
        unsafe {
            imgui_sys::igBeginTabBar(tab_bar_str.as_ptr(), imgui_sys::ImGuiTabBarFlags_None as i32);
        }
        ...
        if tab_is_open {
            ...
            let mut prototypes = HashMap::<core::any::TypeId, Vec<&mut Box<PersistentComponentPrototype>>>::new();

            // Gather all the prototype arcs we will be editing
            let mut storage = resource_map.fetch_mut::<<crate::components::PersistentEntityComponent as Component>::Storage>();
            for entity_handle in entity_handles {
                let comp = storage.get_mut(&entity_handle);
            ...
            for arc in &arcs {
                ...
                for component_prototype in pep.component_prototypes_mut() {
                    ...
                    let component_prototype_type = PersistentComponentPrototype::type_id(&**component_prototype);
                    let prototypes_entry = prototypes.entry(component_prototype_type).or_insert(vec![]);
                    ...
                    unsafe {
                        let component_prototype_ptr : *mut Box<PersistentComponentPrototype> = component_prototype;
