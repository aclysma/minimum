...
impl AcquireCriticalSectionReadLockGuard {
    fn new(
        guard: futures_locks::RwLockReadGuard<()>
    ) -> Self {
        ...
        ResourceId: super::ResourceIdTrait,
{
    ...
    dispatcher.cs_lock().read()
        .map(|guard| {
            AcquireCriticalSectionReadLockGuard::new(guard)
        })
}
...
impl AcquireCriticalSectionWriteLockGuard {
    fn new(
        guard: futures_locks::RwLockWriteGuard<()>
    ) -> Self {
        ...
        ResourceId: super::ResourceIdTrait,
{
    ...
    dispatcher.cs_lock().write()
        .map(|guard| {
            AcquireCriticalSectionWriteLockGuard::new(guard)
        })
}
